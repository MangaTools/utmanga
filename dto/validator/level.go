...
func ValidateLevelSettings(settings dto.LevelSettings) error {
	return validation.ValidateStruct(&settings,
		...
		validation.Field(&settings.DiapasonBlack, validation.Min(0), validation.Max(255-settings.DiapasonWhite)),
		validation.Field(&settings.DiapasonWhite, validation.Min(0), validation.Max(255-settings.DiapasonBlack)),
	)
}
